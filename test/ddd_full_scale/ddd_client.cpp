...
namespace dddynamic_reconfigure {
     ...
     */
    TEST(DDDFullScaleTest, doTest) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
        // ROS init stage
        NodeHandle nh;
        ...
        ServiceClient defaults = nh.serviceClient<dynamic_reconfigure::Reconfigure>("set_parameter_defaults");
        ASSERT_TRUE(defaults.call(srv));

        ServiceClient client_defaults = nh.serviceClient<ddynamic_reconfigure::TutorialParams>("get_param_defaults");
        ddynamic_reconfigure::TutorialParams param_defaults;
        ASSERT_TRUE(client_defaults.call(param_defaults));

        ASSERT_EQ(new_int,param_defaults.response.int_param);
        ASSERT_EQ(new_double,param_defaults.response.double_param);
        ASSERT_EQ(new_string,param_defaults.response.str_param);
        ASSERT_EQ(new_bool,param_defaults.response.bool_param);
        ASSERT_EQ(new_enum,param_defaults.response.enum_param);
    }
}
